...
public class UDGraph
{
	...
	for(int i=0; i<vertices;i++){
		for(int j=0; j<vertices;j++){
			for(int k=0; k<vertices;k++){
				if(this.adjMatrix[i][j]==true && this.adjMatrix[j][k]==true){
  ...
  public UDGraph paths(int length) {
	...
	UDGraph lastGraph = paths(length-1);
	for(int i=0; i<vertices;i++){
		for(int j=0; j<vertices;j++){
			for(int k=0; k<vertices;k++){
				if(lastGraph.adjMatrix[i][j]==true && this.adjMatrix[j][k]==true){
					if(newGraph.adjMatrix[i][k]==false) newGraph.edges++;
					newGraph.adjMatrix[i][k]=true;					
					
					//System.out.print(newGraph.edges+":("+i+","+k+"); ");
				}
			}
		}
	}
