...
public class SList {
  ...
  public void squish() {
	...
	if(head!= null){
		SListNode theNode = head;
		SListNode nextNode = head.next;
		int count = size;
		for(int i = 0; i < size-1; i++){
			if(nextNode.item.equals(theNode.item)){
				theNode.next = nextNode.next;
				nextNode.next = null;
  ...
  public void twin() {
	...
	SListNode thisNode = head;
	//System.out.println(size);
	for(int i = 0; i < size; i++){
		//System.out.println(thisNode.item);
		SListNode newNode = new SListNode(thisNode.item, thisNode.next);
		//newNode.next = thisNode.next;
		thisNode.next = newNode;
		thisNode = newNode.next;
	}
  }
