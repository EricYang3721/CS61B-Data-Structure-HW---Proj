  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
	...
	if(isValidNode()){	
		if(c<=1){
			...
			if(c > this.children()) location = this.children()+1;
			else location = c;
			int itr = 2;
			SibTreeNode ptr = this.firstChild;
			while(itr < location){
				ptr = ptr.nextSibling;
				itr++;
			}
  ...
  public void removeLeaf() throws InvalidNodeException {
	...
	if(isValidNode()){
		if(this == myTree.root()){
			...
			SibTreeNode ptr = this.parent.firstChild;
			if(ptr != this){
				while(ptr.nextSibling != this){
					ptr = ptr.nextSibling;
				}
				if(this.firstChild == null && this.nextSibling == null){
					...
					myTree.size--;
				} else if(this.firstChild == null && this.nextSibling != null) {
					ptr.nextSibling = this.nextSibling;
					this.valid = false;
