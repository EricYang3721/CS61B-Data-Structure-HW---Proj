...
public class PixImage {
	...
	public PixImage(int width, int height) {
		...
		image_width = width;
		image_height = height;
		image = new short[image_height][image_width][3]; // default of the
															// elements in the
															// array is 0;
					...
					for (int i = 0; i < image_height; i++) {
						for (int j = 0; j < image_width; j++) {
							if ((i > 0 && i < image_height - 1) && (j > 0 && j < image_width - 1)) {
								result.image[i][j][k] = (short) ((temp.image[i - 1][j - 1][k] + temp.image[i - 1][j][k]
										+ temp.image[i - 1][j + 1][k] + temp.image[i][j - 1][k] + temp.image[i][j][k]
										+ temp.image[i][j + 1][k] + temp.image[i + 1][j - 1][k]
										+ temp.image[i + 1][j][k] + temp.image[i + 1][j + 1][k]) / 9);
							} else if ((j > 0 && j < image_width - 1) && i == 0) {
								result.image[i][j][k] = (short) ((temp.image[i][j - 1][k] + temp.image[i][j][k]
										+ temp.image[i][j + 1][k] + temp.image[i + 1][j - 1][k]
										+ temp.image[i + 1][j][k] + temp.image[i + 1][j + 1][k]) / 6);
							} else if ((j > 0 && j < image_width - 1) && (i == image_height - 1)) {
								result.image[i][j][k] = (short) ((temp.image[i - 1][j - 1][k] + temp.image[i - 1][j][k]
										+ temp.image[i - 1][j + 1][k] + temp.image[i][j - 1][k] + temp.image[i][j][k]
										+ temp.image[i][j + 1][k]) / 6);
							} else if (j == 0 && (i > 0 && i < image_height - 1)) {
								result.image[i][j][k] = (short) ((temp.image[i - 1][j][k] + temp.image[i - 1][j + 1][k]
										+ temp.image[i][j][k] + temp.image[i][j + 1][k] + temp.image[i + 1][j][k]
										+ temp.image[i + 1][j + 1][k]) / 6);
							} else if (j == image_width - 1 && (i > 0 && i < image_height - 1)) {
								result.image[i][j][k] = (short) ((temp.image[i - 1][j - 1][k] + temp.image[i - 1][j][k]
										+ temp.image[i][j - 1][k] + temp.image[i][j][k] + temp.image[i + 1][j - 1][k]
										+ temp.image[i + 1][j][k]) / 6);
							} else if (i == 0 && j == 0) {
								result.image[i][j][k] = (short) ((temp.image[i][j][k] + temp.image[i][j + 1][k]
										+ temp.image[i + 1][j][k] + temp.image[i + 1][j + 1][k]) / 4);
							} else if (i == 0 && j == image_width - 1) {
								result.image[i][j][k] = (short) ((temp.image[i][j - 1][k] + temp.image[i][j][k]
										+ temp.image[i + 1][j - 1][k] + temp.image[i + 1][j][k]) / 4);
							} else if (i == image_height - 1 && j == 0) {
								result.image[i][j][k] = (short) ((temp.image[i - 1][j][k] + temp.image[i - 1][j + 1][k]
										+ temp.image[i][j][k] + temp.image[i][j + 1][k]) / 4);
							} else if (i == image_height - 1 && j == image_width - 1) {
								result.image[i][j][k] = (short) ((temp.image[i - 1][j - 1][k] + temp.image[i - 1][j][k]
										+ temp.image[i][j - 1][k] + temp.image[i][j][k]) / 4);
							}
						}
					}
				}
	...
	public PixImage sobelEdges() {
		// Replace the following line with your solution.
		long[][][] gx = new long[image_height][image_width][3];
		long[][][] gy = new long[image_height][image_width][3];
		long[][] energy = new long[image_height][image_width];
		long[][][] mirror_matrix = this.ImagMirror();
		// System.out.println("mirror_matrix:");
		// System.out.println(Arrays.deepToString(mirror_matrix));
		PixImage sobel_result = new PixImage(image_width, image_height);

		for (int k = 0; k < 3; k++) {
			for (int i = 0; i < image_height; i++) {
				for (int j = 0; j < image_width; j++) {
					gx[i][j][k] = mirror_matrix[i][j][k] - mirror_matrix[i][j + 2][k] + 2 * mirror_matrix[i + 1][j][k]
							- 2 * mirror_matrix[i + 1][j + 2][k] + mirror_matrix[i + 2][j][k]
							- mirror_matrix[i + 2][j + 2][k];
					gy[i][j][k] = mirror_matrix[i][j][k] + 2 * mirror_matrix[i][j + 1][k] + mirror_matrix[i][j + 2][k]
							- mirror_matrix[i + 2][j][k] - 2 * mirror_matrix[i + 2][j + 1][k]
							- mirror_matrix[i + 2][j + 2][k];

				}
			}
		}
		// System.out.println("gx is :");
		// System.out.println(Arrays.deepToString(gx));
		// System.out.println("gy is :");
		// System.out.println(Arrays.deepToString(gy));

		for (int i = 0; i < image_height; i++) {
			for (int j = 0; j < image_width; j++) {
				energy[i][j] = gx[i][j][0] * gx[i][j][0] + gx[i][j][1] * gx[i][j][1] + gx[i][j][2] * gx[i][j][2]
						+ gy[i][j][0] * gy[i][j][0] + gy[i][j][1] * gy[i][j][1] + gy[i][j][2] * gy[i][j][2];
				sobel_result.image[i][j][0] = mag2gray(energy[i][j]);
				sobel_result.image[i][j][1] = mag2gray(energy[i][j]);
				sobel_result.image[i][j][2] = mag2gray(energy[i][j]);
			}
		}
	...
	public long[][][] ImagMirror() {
		// System.out.println("ImagMirror");
		long[][][] mirror = new long[image_height + 2][image_width + 2][3];

		for (int k = 0; k < 3; k++) {
			for (int i = 0; i < image_height; i++) {
				for (int j = 0; j < image_width; j++) {
					mirror[i + 1][j + 1][k] = (long) image[i][j][k];
		...
		for (int k = 0; k < 3; k++) {
			for (int i = 0; i < image_height; i++) {
				mirror[i + 1][0][k] = (long) image[i][0][k];
				mirror[i + 1][image_width + 1][k] = (long) image[i][image_width - 1][k];
			}
			for (int j = 0; j < image_width; j++) {
				mirror[0][j + 1][k] = (long) image[0][j][k];
				mirror[image_height + 1][j + 1][k] = (long) image[image_height - 1][j][k];
			}
			mirror[0][0][k] = (long) image[0][0][k];
			mirror[0][image_width + 1][k] = (long) image[0][image_width - 1][k];
			mirror[image_height + 1][0][k] = (long) image[image_height - 1][0][k];
			mirror[image_height + 1][image_width + 1][k] = (long) image[image_height - 1][image_width - 1][k];
		}
		// System.out.println(Arrays.deepToString(mirror));
		return mirror;
	}
	...
	public static void main(String[] args) {
		...
		// System.out.println(image1.boxBlur(1).toString());
